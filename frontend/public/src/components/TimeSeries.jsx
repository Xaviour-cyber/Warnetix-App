...
// [{ t, count, high, critical }]  atau
// [{ ts, total, high, critical, label }]  atau
// [{ label, total, high, critical }]
export default function TimeSeries({ series = [] }) {
  const data = useMemo(()=> (series || []).map((row)=>{
    const t  = row.t ?? row.ts ?? row.time ?? null;
    const lb = row.label ?? (t ? fmtLabel(t) : "");
    const total = row.total ?? row.count ?? 0;
    const high  = row.high ?? 0;
    const crit  = row.critical ?? row.crit ?? 0;
    return { label: lb, total, high, critical: crit };
  }), [series]);

  return (
    <div className="card">
      <h3>Trend Aktivitas</h3>
...
function fmtLabel(t){ const ms = t > 1e12 ? t : t * 1000;
